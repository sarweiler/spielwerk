local cs = require("lib/src/crowservice")
...
describe("CrowService", function()
  ...
  it("should initialize a cv output with 0 volts", function()
    local c = cs:new(crow_stub)
    
    c:set_cv_output(1)
    assert.are.equal(crow_stub.output[1].volts, 0)
  end)
  ...
  it("should fire a trigger", function()
    local s = spy.on(crow_stub.output[1], "execute")
    local c = cs:new(crow_stub)

    c:set_trigger_output(1)
    c:fire_trigger(1)
